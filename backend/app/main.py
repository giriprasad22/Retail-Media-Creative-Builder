...
# Run auto-installer FIRST before any other imports (only in development)
def auto_install_packages():
    """Auto-install all required packages on startup (development only)"""
    # Skip auto-install in production (Railway sets RAILWAY_ENVIRONMENT)
    if os.environ.get('RAILWAY_ENVIRONMENT') or os.environ.get('PORT'):
        print("🚂 Running in production mode - skipping auto-install")
        return
...
# Run auto-install when main.py is executed (development only)
if __name__ == "__main__" or (not os.environ.get('RAILWAY_ENVIRONMENT') and not os.environ.get('PORT')):
...
def index():
    ...
    response = send_file(str(FRONTEND_DIR / "index.html"))
    # Prevent caching to ensure users get latest version
...
def editor():
    """Serve editor page."""
    response = send_file(str(FRONTEND_DIR / "editor.html"))
    # Prevent caching to ensure users get latest version
    response.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
    response.headers['Pragma'] = 'no-cache'
    response.headers['Expires'] = '0'
...
@app.route('/test.html')
def test_page():
    """Serve deployment test page."""
    response = send_file(str(FRONTEND_DIR / "test.html"))
    response.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
    response.headers['Pragma'] = 'no-cache'
    response.headers['Expires'] = '0'
    return response
...
def serve_css(filename):
    """Serve CSS files."""
    try:
        response = send_from_directory(str(FRONTEND_DIR / "css"), filename)
        # Prevent caching to ensure updated CSS loads
        response.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
        response.headers['Pragma'] = 'no-cache'
        response.headers['Expires'] = '0'
        return response
...
def serve_js(filename):
    ...
    try:
        response = send_from_directory(str(FRONTEND_DIR / "js"), filename)
        # Prevent caching to ensure updated JS loads
        response.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
        response.headers['Pragma'] = 'no-cache'
        response.headers['Expires'] = '0'
        return response
        ...
        templates = [
            {
                ...
                "description": "Clubcard price promotion template with value tiles"
            },
            {
                ...
                "description": "Low Everyday Price promotional template"
            },
            {
                ...
                "description": "New product launch template with NEW tile"
            },
            {
                "id": "tesco-alcohol-1",
                ...
                "description": "Alcohol promotion with Drinkaware compliance"
            },
            {
                "id": "amazon-product-1",
                ...
                "description": "Amazon product showcase banner"
            },
            {
                "id": "flipkart-sale-1",
                ...
                "description": "Flipkart big sale promotional banner"
            },
            {
                "id": "dmart-grocery-1",
                ...
                "description": "DMart grocery deals template"
            }
        ]
        
        # Filter by platform if specified
        if platform != 'all':
            templates = [t for t in templates if t['category'] == platform]
